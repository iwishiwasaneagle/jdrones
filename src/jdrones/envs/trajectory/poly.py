...
from jdrones.envs import PIDTrajectoryDroneEnv
...
class PolynomialTrajectoryDroneEnv(BaseControlledEnv):
    """
    Fly the drone to a waypoint using a quintic polynomial trajectory
    """
    ...
    def step(self, action: PositionAction) -> Tuple[States, float, bool, bool, dict]:
        ...
        traj = Trajectory(
            start_pos=self.env.state.pos,
            start_vel=self.env.state.vel,
            start_acc=(0, 0, 0),
            dest_pos=action,
            dest_vel=(0, 0, 0),
            dest_acc=(0, 0, 0),
            T=euclidean_distance(*self.env.state.pos[:2], *action[:2]) / 0.2,
        )
        ...
        for i in count():
            ...
            ti = self.dt * i
            if ti <= traj.T:
                tgt_pos = traj.position(i * self.dt)
            else:
                tgt_pos = action

            cur_pos_b, tgt_pos_b = self._position_world_to_body(
                cur_pos, tgt_pos, cur_rpy
            )
...
if __name__ == "__main__":
    ...
    env = gymnasium.make(
        "PolynomialTrajectoryDroneEnv-v0",
        dt=dt,
        initial_state=initial_state,
