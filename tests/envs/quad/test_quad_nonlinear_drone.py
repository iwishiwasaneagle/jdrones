...
def test_zero_input(vec_omega, quadnonlineardroneenv):
    """
    Expect it to drop like a stone
    """
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
    assert np.allclose(np.sign(obs.vel), (0, 0, -1))


@pytest.mark.integration
@LOW_INPUT
def test_low_input(vec_omega, quadnonlineardroneenv):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
    assert np.allclose(np.sign(obs.vel), (0, 0, -1))
...
@pytest.mark.integration
@LARGE_INPUT
def test_large_input(vec_omega, quadnonlineardroneenv):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
    assert np.allclose(np.sign(obs.vel), (0, 0, 1))
...
def test_hover_input(vec_omega, quadnonlineardroneenv):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
...
@pytest.mark.integration
@ROLL_INPUT
def test_roll_input(vec_omega, quadnonlineardroneenv, exp):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
...
@pytest.mark.integration
@PITCH_INPUT
def test_pitch_input(vec_omega, quadnonlineardroneenv, exp):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
    assert np.allclose(np.sign(obs.ang_vel), exp)
...
@pytest.mark.integration
@YAW_INPUT
def test_yaw_input(vec_omega, equilibrium_prop_rpm, quadnonlineardroneenv, exp):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
    assert np.allclose(np.sign(obs.ang_vel), exp)
...
def test_vel_from_rot(vec_omega, quadnonlineardroneenv, exp):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(0.99 * vec_omega)
...
def test_pos_from_vel(vec_omega, quadnonlineardroneenv, velocity):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
...
@pytest.mark.integration
@RPY_FROM_ANG_VEL
def test_rpy_from_ang_vel(vec_omega, quadnonlineardroneenv, angular_velocity):
    quadnonlineardroneenv.reset()
    obs, *_ = quadnonlineardroneenv.step(vec_omega)
...
@pytest.mark.integration
@INPUT_TO_ROT
def test_input_to_rot(seed, quadnonlineardroneenv, action, k_Q, ang_vel_sign):
    """
    Step input over a short time will give a good insight if the drone is behaving
    as expected
    """
    quadnonlineardroneenv.reset(seed=seed)
    for _ in range(5):
        obs, *_ = quadnonlineardroneenv.step(action * 100)
